...
import { createModeArmer } from './mode.js'
...
const SETTINGS_DEFAULTS = Object.freeze({ defaultMode: 'local', rememberLastChoice: true })
...
export function apply(ctx, config = {}) {
  ...
  const modeArmer = createModeArmer({
    defaultMode: () => readSettings().defaultMode,
    inject: (agent, name) => {
  ...
  ctx.on('agent/session-start', ({ agent, source }) => {
    modeArmer.sessionStart(agent, source)
  })
  ctx.on('agent/inbox/inserted', ({ agent, message }) => {
    modeArmer.inboxInserted(agent, message)
  })
  ctx.on('agent/disposed', ({ agent }) => {
    modeArmer.disposed(agent)
  })

  registerWorktreeCommand(ctx, manager, { arm: modeArmer.arm, disarm: modeArmer.disarm })
}